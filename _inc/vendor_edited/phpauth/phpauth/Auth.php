...
class Auth
{
	...
	public function register($email, $password, $repeatpassword, $params = Array(), $captcha = NULL, $sendmail = NULL, $createCompany = false)
	{
		$return['error'] = true;
		$block_status = $this->isBlocked();
		...
		global $database;
		if($createCompany) {
			$database->insert( 'companies', [ 'name' => 'new company' ] );
			$company_id = $database->id();
			if ( ! $company_id ) {
				$return['message'] = "COMPANY IN DATABASE NOT GENERATED CORRECTLY";
		...
		$database->insert( $this->config->table_user_profiles, [ 'name' => 'admin', 'company_id' => $company_id ] );
	...
	{
		$query = $this->dbh->prepare("SELECT active_profile FROM {$this->config->table_users} WHERE id = ?");
	...
	protected function addUser($email, $password, $params = array(), &$sendmail, $profile_id)
	{
		$return['error'] = true;
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET email = ?, password = ?, isactive = ?, active_profile = ? {$setParams} WHERE id = ?");
